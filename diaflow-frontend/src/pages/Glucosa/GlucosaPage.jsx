...
function GlucosaPage() {
  ...
  const dataGrafico = useMemo(() => {
    ...
    return copia.map((g) => ({
      ...g,
      fechaLabel: new Date(g.medido_en).toLocaleString("es-ES"),
    }));
  }, [glucemias]);
    ...
    <div className="space-y-4">
      ...
      <header>
        ...
        <p className="text-sm text-slate-500">
          Registra tus mediciones y visualiza la evolución en el tiempo.
        </p>
      </header>

      {/* Filtros */}
      <div className="mb-2 bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <button
            ...
            className="text-sm border px-3 py-1 rounded text-slate-700 hover:bg-slate-100"
          >
            Limpiar filtros
          </button>
        </div>
      ...
      {/* Formulario alta / edición */}
      <div className="mb-4 bg-white border border-slate-200 rounded-xl p-4 shadow-sm max-w-xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          ...
          <div className="flex gap-2 mt-2">
            <button
              ...
              className="bg-blue-600 text-white text-sm px-4 py-1 rounded hover:bg-blue-700 disabled:opacity-60"
            >
            ...
            {editingId && (
              <button
                type="button"
                onClick={cancelEdit}
                className="border text-sm px-3 py-1 rounded text-slate-700 hover:bg-slate-100"
              >
                Cancelar
              </button>
            )}
      ...
      {dataGrafico.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm w-full h-64">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={dataGrafico}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="valor_mg_dl" dot={{ r: 2 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
